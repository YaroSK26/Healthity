...
const food = [
  ...
  const [filteredActivities, setFilterActivities] = useState(food);
    const [activitiesSelections, setFoodSelections] = useState({});
...
const handleAddFood = async (e, item) => {
  ...
  try {
    const optionHundred =
      parseFloat(activitiesSelections[item.id]?.option || item.opt1) / 100;
    const quantityFinal = parseFloat(activitiesSelections[item.id]?.quantity || 1);
    ...
    const data = {
      ...
      option: parseFloat(activitiesSelections[item.id]?.option || item.opt1),
    ...
    const res = await axios.post("/api/activites", data);
          ...
          <form key={item.title} onSubmit={(e) => handleAddFood(e, item)}>
            <div className="w-60 h-[350px] border border-sky-500 rounded-xl flex flex-col items-center gap-3">
              ...
              <div className="flex flex-col">
                <div className="text-xl font-bold mb-3"> -56 kcal </div>
                <select
