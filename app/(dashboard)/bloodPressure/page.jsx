    ...
    <div className="flex flex-col justify-center items-center mt-5 gap-5">
      <form onSubmit={handleSubmit}>
        ...
        <div className="w-52  flex flex-col  justify-center items-center m-auto">
          ...
          <button  className="bg-black p-1 mt-2 rounded-lg text-white w-full">
            Submit
          </button>
        </div>
      </form>
        ...
        realProduct.map((e) => (
          <div key={e._id} className="static">
            <p className="m-0 p-0 static flex gap-3">
              {e.date}:
              <span className="font-bold">{e.bloodPressureUp} / {e.bloodPressureDown}</span>
              <Link href={"/edit-bloodPressure/" + e._id} className="hover:cursor-pointer">
                <Pencil />
              </Link>
              <Link href={"/delete-bloodPressure/" + e._id} className="hover:cursor-pointer">
                <Trash2 color="#ff0000" />
              </Link>
            </p>
          </div>
